...
void calculateRangeHighsAndLows()
{
   if(range.withinRangeTime(lastTick.time))
   {
      ...
      if(lastTick.bid + (rangeBuffer*Point) > range.highOfRange){
         range.highOfRange = lastTick.bid + (rangeBuffer*Point);
         drawRangeBox();
      }

      if(lastTick.bid - (rangeBuffer*Point) < range.lowOfRange){
         range.lowOfRange = lastTick.bid - (rangeBuffer*Point);
         drawRangeBox();
      }
   }
}
